...
/** 文字帖判断：有非空 text、或有 cw、或有 poll 即算有内容（引用转贴带自己的 text 也算） */
export function hasTextContent(n) {
  return !!(n && (((n.text != null) && String(n.text).trim() !== '') || n.cw || n.poll));
}

/** 笔记是否通过内容类型过滤（crawl 客户端逐条过滤用；contentType 缺省按 'image'） */
export function noteMatchesType(note, contentType, f = {}) {
  const ct = contentType || 'image';
  if (ct === 'all') return true;
  if (ct === 'article') return hasTextContent(note);
  if (ct === 'renote') return !!note.renoteId;
  return noteImages(note, f).length > 0; // image（缺省）
}
...
/** files 裁剪：保留档案需要的 11 个字段（顶层与 renote 子对象共用） */
  ...
  const out = {
    ...
    renote: note.renote
      ? {
          ...
          cw: note.renote.cw ?? null,
          user: note.renote.user
            ? { username: note.renote.user.username, host: note.renote.user.host ?? null, name: note.renote.user.name ?? null }
            : null,
          files: trimFiles(note.renote.files),
        }
      : null,
    files: trimFiles(note.files),
    localOnly: !!note.localOnly,
  };
  // 投票：仅在原笔记带 poll 时产出该键
  if (note.poll) {
    out.poll = {
      multiple: !!note.poll.multiple,
      expiresAt: note.poll.expiresAt ?? null,
      choices: (note.poll.choices || []).map((c) => ({ text: c.text ?? '', votes: c.votes || 0, isVoted: !!c.isVoted })),
    };
  }
  return out;
}
