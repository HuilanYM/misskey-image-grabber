...
const TOKEN_RE = new RegExp(
  [
    '(`[^`\\n]+`)', // 行内代码
    '(\\[[^\\]\\n]+\\]\\(https?:\\/\\/[^)\\s]+\\))', // [text](url) 方括号链接
    '(https?:\\/\\/[^\\s<>"\']+)', // URL
    '(:([A-Za-z0-9_+\\-.]+)(@[A-Za-z0-9_\\-.]+)?:)', // 全表情 :name@host: / :name:
    '(@([A-Za-z0-9_\\-.]+)(@[A-Za-z0-9_\\-.]+)?)', // 提及
    '(\\*\\*[^*\\n]+\\*\\*)', // 粗体
    '(~~[^~\\n]+~~)', // 删除线
    '(#[^\\s#,;:!?()\\[\\]{}<>"\']+)', // 话题标签（宽松，不含句点）
  ].join('|'),
...
export function renderText(text, emojiRef) {
  ...
  if (!src) return '';
  // 代码块 ```...```（跨行）先摘出占位，避免被逐行处理拆散；内容转义后原样展示
  const blocks = [];
  const withBlocks = src.replace(/```(?:[A-Za-z0-9_+\-.]*)\n?([\s\S]*?)```/g, (m, code) => {
    blocks.push('<pre class="codeblock"><code>' + escapeHtml(code.replace(/\n$/, '')) + '</code></pre>');
  ...
  // <center>…</center>（MFM 居中）：还原转义后的标签为样式容器
  let html = outLines.join('<br>').replace(/&lt;center&gt;/g, '<div class="center">').replace(/&lt;\/center&gt;/g, '</div>');
  html = html.replace(/\u0000B(\d+)\u0000/g, (m, i) => blocks[+i] || '');
  return html;
}

function renderInline(escaped, emojiRef) {
  return escaped.replace(TOKEN_RE, (m, code, mdlink, url, emojiFull, eName, eHost, mention, mName, mHost, bold, strike, tag) => {
    if (code) return '<code class="inlinecode">' + code.slice(1, -1) + '</code>';
    if (mdlink) {
      const mm = mdlink.match(/^\[([^\]]+)\]\((https?:\/\/[^)\s]+)\)$/);
      return '<a href="' + escapeHtml(mm[2].replace(/&amp;/g, '&')) + '" target="_blank" rel="noopener noreferrer">' + renderInline(mm[1], emojiRef) + '</a>';
    }
    if (url) {
...
function noteCard(note, refFor, emojiRef, revealSens = false) {
  ...
  const noteUrl = note.url || ('https://misskey.io/notes/' + note.id);
  const displayName =
  ...
  const badge = (t) => '<span class="badge" title="' + escapeHtml(t) + '">' + escapeHtml(t) + '</span>';
  const badges =
    (note.replyId ? badge(A('aBadgeReply')) : '') +
    (note.renoteId && note.text ? badge(A('aBadgeRenote')) : '') +
    (note.renoteId && !note.text ? badge(A('aBadgeBoost')) : '') +
    (note.localOnly ? badge(A('aBadgeLocal')) : '');
  ...
  // 投票（静态只读渲染：文本 + 票数条）
  let pollHtml = '';
  if (note.poll && Array.isArray(note.poll.choices) && note.poll.choices.length) {
    const maxV = Math.max.apply(null, note.poll.choices.map((c) => c.votes || 0).concat([1]));
    pollHtml =
      '<div class="poll">' +
      note.poll.choices
        .map((c) => {
          ...
          return '<div class="poll-row"><div class="poll-bar" style="width:' + pct + '%"></div><span class="poll-txt">' + escapeHtml(c.text) + '</span><span class="poll-n">' + v + '</span></div>';
        })
        .join('') +
      '</div>';
  }
  ...
  return (
    '<article class="note' + (imgs.length ? '' : ' noimg') + '" id="n-' + escapeHtml(note.id) + '" data-t="' + escapeHtml(search).replace(/"/g, '&quot;') + '">' +
    avatar +
    '<div class="nbody">' +
    '<div class="nhead">' +
    displayName +
...
/* 文字笔记 / MFM 扩展（v0.3.0） */
.note.noimg{padding:15px 18px}
.note.noimg .text{font-size:15px}
.inlinecode{background:var(--panel2);border:1px solid var(--line);border-radius:5px;padding:0 5px;font-family:ui-monospace,Consolas,monospace;font-size:.92em}
pre.codeblock{background:var(--panel2);border:1px solid var(--line);border-radius:10px;padding:10px 12px;overflow-x:auto;margin:6px 0}
pre.codeblock code{font-family:ui-monospace,Consolas,monospace;font-size:12.5px;color:var(--text)}
.center{text-align:center}
.poll{margin:8px 0 2px;display:grid;gap:5px;max-width:520px}
.poll-row{position:relative;display:flex;align-items:center;gap:8px;background:var(--panel2);border:1px solid var(--line);border-radius:8px;padding:5px 10px;overflow:hidden}
.poll-bar{position:absolute;left:0;top:0;bottom:0;background:color-mix(in srgb,var(--accent) 26%,transparent);border-radius:8px 0 0 8px}
.poll-txt,.poll-n{position:relative}
.poll-n{margin-left:auto;color:var(--dim);font-size:12px}
...
.mp-btn.on{border-color:var(--accent);color:var(--accent)}
...
.mthumb img{display:block;width:100%;height:100%;object-fit:cover;background:var(--panel2)}
.mthumb:hover img{opacity:.85}
.mthumb.on{outline:3px solid var(--accent);outline-offset:-3px}
/* 时间视图：月栏吸顶 + 月内行对齐（justified）；mgrid 切 block，缩略图宽高由 JS 内联给出 */
#mediaGrid.time{display:block}
.mbar{position:sticky;top:-1px;z-index:2;background:var(--panel);margin:0 -12px;padding:7px 14px;font-weight:600;font-size:12.5px;border-bottom:1px solid var(--line)}
.mrow{display:flex;gap:10px;margin-bottom:10px;align-items:flex-start}
#mediaGrid.time .mthumb{flex:0 0 auto}
#mediaGrid.time .mthumb img{width:100%;height:100%;object-fit:cover}
#mediaGrid.time .mthumb .mtime{opacity:1} /* 时间视图日期常显：这本来就是"按时间看"的模式 */
.mthumb.sens::after{content:"${escapeHtml(A('aMthumbBadge'))}";position:absolute;top:8px;right:8px;font-size:11px;background:rgba(0,0,0,.72);color:#ffd23e;border-radius:6px;padding:2px 7px}
...
<header class="profile">
  ...
  <div class="pmeta">${escapeHtml(A('aCrawledAt'))} ${escapeHtml(meta.crawlAt)} · ${escapeHtml(A('aOfflineGen'))}${meta.note ? ' · ' + escapeHtml(meta.note) : ''}</div>
</header>
<div class="toolbar">
...
<div class="lightbox" id="lb"><button class="lb-close" title="${escapeHtml(A('aCloseTitle'))}"><svg class="ic"><use href="#i-x"/></svg></button><div class="lb-cap" id="lbCap"></div><button class="lb-btn prev" title="←">‹</button><img id="lbImg" alt=""><button class="lb-btn next" title="→">›</button><div class="lb-stripzone" id="lbStripZone"><button class="lb-strip-handle" id="lbStripHandle" type="button" title="${escapeHtml(A('aStripTip'))}">${escapeHtml(A('aStripHandle'))}</button><div class="lb-strip" id="lbStrip"></div></div></div>
<button class="toTop" id="toTop" title="${escapeHtml(A('aToTopTitle'))}"><svg class="ic"><use href="#i-up"/></svg></button>
<aside id="mediaPanel" aria-label="${escapeHtml(A('aMpTitle'))}">
  <div class="mp-head">
    ...
    <button class="mp-btn" id="mpView" type="button" title="${escapeHtml(A('aMpViewTitle'))}"><svg class="ic"><use href="#i-calendar"/></svg>${escapeHtml(A('aMpView'))}</button>
...
(function(){
  ...
  function applyFilter(){
    ...
    var unit = ${JSON.stringify(A('aNotesUnit'))};
    count.textContent = n + ' / ' + notes.length + (unit ? ' ' + unit : '');
  }
  ...
  var MPU = ${JSON.stringify(A('aMpUnit'))};
  document.getElementById('mediaBtnCnt').textContent = imgs.length + MPU;
  if (!imgs.length) { // 全文字档案：媒体抽屉没有内容，隐藏两个入口避免"0 张"空壳
    document.getElementById('mediaBtn').style.display = 'none';
    document.getElementById('mediaTab').style.display = 'none';
  }
  document.getElementById('mpCount').textContent = imgs.length + MPU;
  // 双视图：flow=瀑布流（原比例、零裁切） / time=月分组+行对齐（位置=发布顺序）；偏好持久化
  var mview = 'flow'; try { if (localStorage.getItem('mg-media-view') === 'time') mview = 'time'; } catch (e) {}
  ...
  var thumbs = imgs.map(function(img, i){
    ...
    var ar = 1.5; try { ar = parseFloat(box.style.aspectRatio) || 1.5; } catch (e) {} // 构建期宽高比（w/h，来自 file.properties）
    var ym = 0;
    try { var d0 = new Date(timeEl ? timeEl.getAttribute('datetime') : NaN); if (!isNaN(d0)) ym = d0.getFullYear() * 100 + d0.getMonth() + 1; } catch (e) {}
    var b = document.createElement('button');
    b.type = 'button'; b.className = 'mthumb'; b.title = (i + 1) + ' / ' + imgs.length;
    b.style.aspectRatio = String(ar); // 空间预留：未加载也占住真实比例的盒子，列不塌缩（时间视图内联宽高会覆盖它）
    var t = document.createElement('img');
    t.src = img.getAttribute('src'); t.loading = 'lazy'; t.decoding = 'async'; t.alt = '';
    var ratio = 1; // h/w 估计值：构建期比例先给初值（首屏列高即精准），加载后按真值更新——均不重排
    try { var a0 = parseFloat(box.style.aspectRatio); if (a0 > 0) ratio = 1 / a0; } catch (e) {}
    ...
    return { el: b, ratio: function(){ return ratio; }, aspect: ar, ym: ym };
  });
  // 月分组（thumbs 已按首发时间降序 → 组天然连续）
  var monthGroups = [];
  thumbs.forEach(function(th){
    var g = monthGroups[monthGroups.length - 1];
    if (!g || g.ym !== th.ym) { g = { ym: th.ym, items: [] }; monthGroups.push(g); }
    g.items.push(th);
  });
  function layoutFlow(){
    ...
    mcols = [];
    var GAP = 10;
    var colW = (mgrid.clientWidth - (cols - 1) * GAP) / cols; // 与 flex 均分一致的列宽（估值按像素算才准）
    var heights = [], colItems = [];
    for (var c = 0; c < cols; c++) {
      var col = document.createElement('div'); col.className = 'mcol';
      mgrid.appendChild(col); mcols.push(col); heights.push(0); colItems.push([]);
    }
    thumbs.forEach(function(th){
      var c = 0; for (var k = 1; k < heights.length; k++) if (heights[k] < heights[c]) c = k;
      th.el.style.width = ''; th.el.style.height = ''; // 清掉时间视图的内联尺寸
      mcols[c].appendChild(th.el);
      colItems[c].push(th);
      heights[c] += colW * th.ratio() + GAP; // 像素化估值：列宽×比例 + 间距
    });
    // 尾部重平衡：贪心没有回头能力，收尾残差会在底部留一个大空洞——
    // 从最高列尾部取后缀（最多 4 项）移向最矮列，选使两列差最小的后缀，直到无可改善
    for (;;) {
      var hi = 0, lo = 0, k;
      for (k = 1; k < heights.length; k++) { if (heights[k] > heights[hi]) hi = k; if (heights[k] < heights[lo]) lo = k; }
      var d = heights[hi] - heights[lo];
      if (d <= 0) break;
      var items = colItems[hi];
      var bestJ = 0, bestD = d, sum = 0;
      for (var j = 1; j <= 4 && j <= items.length; j++) {
        sum += colW * items[items.length - j].ratio() + GAP;
        var nd = Math.abs(d - 2 * sum);
        if (nd < bestD) { bestD = nd; bestJ = j; }
      }
      if (!bestJ) break;
      sum = 0;
      for (var m = 0; m < bestJ; m++) {
        var t = items.pop();
        sum += colW * t.ratio() + GAP;
        colItems[lo].push(t);
        mcols[lo].appendChild(t.el);
      }
      heights[hi] -= sum;
      heights[lo] += sum;
    }
  }
  // 时间视图：月内行对齐（justified，Flickr 式贪心 + 容差带；末行按目标行高靠左，不拉伸）
  var lastTimeW = 0;
  function layoutTime(){
    var W = mgrid.clientWidth; if (!W) return;
    if (W === lastTimeW && mgrid.classList.contains('time') && mgrid.firstChild) return; // 宽度未变且已建：不重建
    lastTimeW = W;
    while (mgrid.firstChild) mgrid.removeChild(mgrid.firstChild);
    mgrid.classList.add('time');
    var GAP = 10, TOL = 0.22;
    var H0 = Math.max(150, Math.min(190, Math.round(W * 0.48))); // 行高自适应抽屉宽：与瀑布流列宽同级、够阅览的尺寸
    var maxAR = (W - GAP) / (H0 * (1 - TOL)); // 行总宽高比接受带上沿
    monthGroups.forEach(function(g){
      var bar = document.createElement('div'); bar.className = 'mbar';
      var s;
      try { s = new Date(Math.floor(g.ym / 100), g.ym % 100 - 1, 1).toLocaleDateString(L === 'en' ? 'en-US' : L, { year: 'numeric', month: 'long' }); }
      catch (e) { s = Math.floor(g.ym / 100) + '-' + (g.ym % 100); }
      bar.textContent = s + ' · ' + g.items.length + MPU;
      mgrid.appendChild(bar);
      var row = [], rowAR = 0;
      function flush(last){
        if (!row.length) return;
        var n = row.length;
        var fitH = (W - (n - 1) * GAP) / rowAR;
        var rowH = last ? Math.min(H0, fitH) : Math.min(fitH, H0 * (1 + TOL) + 40);
        var r = document.createElement('div'); r.className = 'mrow';
        row.forEach(function(th){
          th.el.style.width = Math.round(rowH * th.aspect) + 'px';
          th.el.style.height = Math.round(rowH) + 'px';
          r.appendChild(th.el);
        });
        mgrid.appendChild(r);
        row = []; rowAR = 0;
      }
      g.items.forEach(function(th){
        row.push(th); rowAR += th.aspect;
        if (rowAR > maxAR) {
          var withIt = (W - (row.length - 1) * GAP) / rowAR;                  // 收下这张的行高
          var withoutIt = (W - (row.length - 2) * GAP) / (rowAR - th.aspect); // 留给下行的行高
          if (Math.abs(withIt - H0) <= Math.abs(withoutIt - H0)) {
            flush(false); // 收下这张并封行
          } else {
            var keep = row.pop(); rowAR -= th.aspect;
            flush(false);          // 不收这张，先封前面的行
            row = [keep]; rowAR = keep.aspect; // 被拒的图开启下一行（不能丢！）
          }
        }
      });
      flush(true);
  ...
  function layout(){
    if (mview === 'time') { layoutTime(); return; }
    mgrid.classList.remove('time');
    layoutFlow();
  }
  ...
  var mpViewBtn = document.getElementById('mpView');
  function applyView(skipMark){
    mpViewBtn.classList.toggle('on', mview === 'time');
    try { localStorage.setItem('mg-media-view', mview); } catch (e) {}
    mcols = []; lastTimeW = 0; // 切换视图：强制重建（用户主动的一锤子事件）
    layout();
    if (!skipMark && cur >= 0 && mediaOn) markThumb(cur);
  }
  mpViewBtn.addEventListener('click', function(){ mview = mview === 'time' ? 'flow' : 'time'; applyView(false); });
  applyView(true);
  // 灯箱底部胶片条
  ...
  function jumpAnchor(art) {
    ...
    art.querySelectorAll('details.cw').forEach(function (d) { d.open = true; });
    centerNote(art);
    art.classList.add('flash');
    setTimeout(function () { art.classList.remove('flash'); }, 1800);
...
export function buildDataJson({ user, notes, meta, items }) {
  const payload = {
    generator: 'Misskey Image Grabber v0.3.0', // 与 manifest 版本同步更新
