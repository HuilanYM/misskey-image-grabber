...
- **ワンクリック取得**：misskey.io のユーザーページで注入ボタンを押すだけ。**すべて / 画像のみ / 文章 / リノート**の 4 種類のコンテンツタイプで、タイムライン全体（文字投稿・返信・チャンネルノート含む）を取得
- **マルチサイト**：misskey.io に加えて **baraag.net**（Mastodon）に対応——`@ユーザー名@baraag.net` を入力するだけ。同じ混合アーカイブ管線を使います
...
- **オフラインアーカイブページ**：文字と画像の混在タイムライン（文字ノート・リノートカード・投票スナップショット）、メディアドロワー、ライトボックス（ホイール切替）、カレンダーとアクティビティ軸ナビ、全文検索、センシティブマスク（misskey 式 CSS ブラー）、ダーク/ライトテーマ、日英中 3 言語 UI
