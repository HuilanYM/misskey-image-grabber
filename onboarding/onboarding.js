...
const I18N = {
  'zh-CN': {
    ...
    wn0219: '「快速」预设节奏收紧：请求间隔加倍、补上批次休息——在速度与对实例的友好之间重新平衡',
    ...
    wn0226: '修复图片栏（瀑布流）底部的空洞：排布完成后自动做尾部平衡，并为未加载的图片预留空间——滚动中不再塌缩、底部不再缺一块',
    wn0300: '全新 v0.3.0：抓取范围扩展到整个时间流——目标用户下新增「全部/仅图片/文章/转发」四种内容类型，档案支持纯文字笔记、转发卡与投票',
    wn0400: '新增 baraag.net（Mastodon）支持：目标用户输入 @用户名@baraag.net 即可抓取，四种内容类型与混合档案全部可用',
  ...
  ja: {
    ...
    wn0218: '初回インストール時に UI 言語をブラウザの言語に自動追従（日本語環境なら日本語）。いつでも手動切替可能',
    wn0219: '「高速」プリセットのペースを引き締め：リクエスト間隔を倍にし、バッチ休憩を追加。インスタンスへの負荷と速度のバランスを再調整',
    ...
    wn0226: 'メディアドロワー（瀑布流）下部の空洞を修正：配置後に末尾バランスを自動調整し、未読み込み画像のスペースを事前確保——スクロール中の崩れや底の欠けを解消',
    wn0300: 'v0.3.0：取得範囲をタイムライン全体へ拡張——対象ユーザーの下に「すべて/画像のみ/文章/リノート」の 4 種類を追加。アーカイブは文字ノート・リノートカード・投票に対応',
    wn0400: 'baraag.net（Mastodon）に対応：対象ユーザーに @ユーザー名@baraag.net と入力するだけで取得可能。4 つのコンテンツタイプと混合アーカイブがそのまま使えます',
  ...
  en: {
    ...
    wn0218: 'UI language now follows the browser language on first install (switchable anytime)',
    wn0219: 'Tightened the "fast" preset pacing: doubled request intervals and added batch rests, rebalancing speed vs. instance load',
    ...
    wn0226: 'Fixed the cavity at the bottom of the flow media drawer: tail balancing after placement and reserved space for unloaded images — no more mid-scroll collapse or missing chunk at the end',
    wn0300: 'v0.3.0: grabbing now covers the whole timeline — four content types (All / Images only / Posts / Renotes) under the target user, and archives support text notes, renote cards and polls',
    wn0400: 'Added baraag.net (Mastodon) support: enter @username@baraag.net as the target user — all four content types and mixed archives work out of the box',
